...
def main():
    ...
    X = data.drop(['Age', 'GoodCustomer'], axis=1).values
    y= data['GoodCustomer'].replace({-1: 0, 1: 1}).values
    sensitive_attr = data['Gender'].values
    ...
    ax.set_title('Pareto Front of NSGA-II Optimization for German Dataset')
    plt.tight_layout()
    plt.savefig('pareto_front_pymoo_nn_epoch_german.png')
    ...
    with open("results/metrics_german.txt", "w") as f:
    ...
    def analyze_explanations_by_group(model, X, sensitive):
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig('feature_importance_by_group_shap_lime_German.png')
    ...
    # Compute stability scores for each instance.
        ...
        plt.tight_layout()
        plt.savefig('stability_distribution_by_group_combined_German.png')
