    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i in range(n_points):
            ...
            try:
                ...
                f[i, 0] = -auc
                f[i, 1] = explainability_weight * gesd
                f[i, 2] = fairness_weight * dp_diff
...
def main():
    data = pd.read_csv(r"C:\Users\olasu\Downloads\math.csv")
    ...
    X = data.drop(['age', 'G3'], axis=1).values
    y = data['G3'].values
    sensitive_attr = data['sex'].values
    ...
    ax.set_title('Pareto Front of NSGA-II Optimization for Math Dataset')
    plt.tight_layout()
    plt.savefig('pareto_front_pymoo_nn_epoch.png')
    ...
    for i in range(len(Fs)):
        ...
        utility = 0.6 * norm_auc + explainability_weight * norm_gesd + fairness_weight * norm_dp
    ...
    with open("results/metrics_math.txt", "w") as f:
    ...
    def analyze_explanations_by_group(model, X, sensitive):
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig('feature_importance_by_group_shap_lime_math.png')
    ...
    def visualize_stability_distributions(model, X, sensitive):
        ...
        X_sample = X[indices]
        sensitive_sample = sensitive[indices]
    
        # Helper functions for explanations.
        def get_shap_explanation(instance):
            shap_vals = explainer(instance.reshape(1, -1))
            # Assume explainer returns a SHAP values object with .values attribute.
            shap_vector = np.squeeze(shap_vals.values)
            if shap_vector.ndim != 1:
                shap_vector = shap_vector.flatten()
            return shap_vector
        ...
        def get_lime_explanation(instance):
            ...
            if lime_map:
                ...
                lime_values_list = []
            lime_vector = np.zeros(X.shape[1])
            for feat_idx, weight in lime_values_list:
                lime_vector[feat_idx] = weight
            return lime_vector

    # Compute stability scores for each instance.
        for instance, group in zip(X_sample, sensitive_sample):
            base_shap = get_shap_explanation(instance)
            base_lime = get_lime_explanation(instance)
        
            shap_scores = []
            lime_scores = []
            for _ in range(n_perturbations):
                perturbed = instance + np.random.normal(0, perturbation_scale, size=instance.shape)
                perturbed_shap = get_shap_explanation(perturbed)
                perturbed_lime = get_lime_explanation(perturbed)
                distance_shap = np.linalg.norm(base_shap - perturbed_shap, ord=1)
                distance_lime = np.linalg.norm(base_lime - perturbed_lime, ord=1)
                score_shap = 1 / (1 + distance_shap)
                score_lime = 1 / (1 + distance_lime)
                shap_scores.append(score_shap)
                lime_scores.append(score_lime)
        ...
        combined_stability = (stabilities_shap + stabilities_lime) / 2.0
    
        # Plot histograms: one for SHAP, one for LIME, and one for the combined stability.
        unique_groups = np.unique(groups)
        fig, axs = plt.subplots(1, 3, figsize=(18, 6))
    
        for group in unique_groups:
            group_shap = stabilities_shap[groups == group]
            axs[0].hist(group_shap, alpha=0.5, label=f'Group {group}', bins=15)
        axs[0].set_xlabel('SHAP Stability Score')
        axs[0].set_ylabel('Count')
        axs[0].set_title('SHAP Stability Distribution')
        axs[0].legend()
    
        for group in unique_groups:
            group_lime = stabilities_lime[groups == group]
            axs[1].hist(group_lime, alpha=0.5, label=f'Group {group}', bins=15)
        axs[1].set_xlabel('LIME Stability Score')
        axs[1].set_ylabel('Count')
        axs[1].set_title('LIME Stability Distribution')
        axs[1].legend()
    
        for group in unique_groups:
            group_combined = combined_stability[groups == group]
            axs[2].hist(group_combined, alpha=0.5, label=f'Group {group}', bins=15)
        axs[2].set_xlabel('Combined Stability Score')
        axs[2].set_ylabel('Count')
        axs[2].set_title('Combined (SHAP+LIME) Stability Distribution')
        axs[2].legend()
    
        plt.tight_layout()
        plt.savefig('stability_distribution_by_group_combined_Math.png')
    
        # Print mean and std for each group.
        for group in unique_groups:
            grp_shap = stabilities_shap[groups == group]
            grp_lime = stabilities_lime[groups == group]
            grp_comb = combined_stability[groups == group]
